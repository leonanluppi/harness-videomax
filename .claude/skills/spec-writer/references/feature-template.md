...
Each feature produces THREE files in a subfolder:
...
| `contract.md` | Behavior contract | Stack-agnostic GWT items per verification surface, with a coverage manifest tying every PRD §9 AC to its covering items. Read by the implementing and evaluating agents. See `contract-template.md` for the full format. |
