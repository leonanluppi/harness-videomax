...
Progress file consistency (validates the data that PHASE 5 will write):
- [ ] Every feature ID in Section 6 will appear as a key in `features` of the progress file
- [ ] `dependencies` per feature equals the Dependencies column in Section 8
- [ ] `priority` per feature equals the Priority column in Section 8
- [ ] `wave` per feature equals the wave computed in Execution Waves
...
### PHASE 5: Save PRD and Progress File
...
**Step 2 — Build the progress file payload**

Construct the JSON payload for `{PROGRESS_PATH}` following the schema in the "PROGRESS FILE SCHEMA" section below. Use the current timestamp in RFC 3339 UTC (e.g., `2026-05-02T14:30:00Z`); call it `now`.

Branch on whether the file already exists:

**Case A — `{PROGRESS_PATH}` does NOT exist (first generation):**
- Set `schema_version: 1`, `prd_path: {PRD_PATH}`, `generated_at: now`.
- For every feature ID in the Section 8 dependency table, create an entry with:
  - `name`, `priority`, `wave`, `dependencies` copied from the PRD
  - `status: "pending"`, `cycles: 0`
  - `failure_reason: null`, `report_path: null`, `started_at: null`, `completed_at: null`
  - `updated_at: now`

**Case B — `{PROGRESS_PATH}` exists (PRD regeneration / merge):**
- Read and parse the existing JSON. If parse fails, see EDGE CASES ("Existing progress file is corrupt").
- If `schema_version` in the existing file is greater than `1`, stop and report — do not downgrade.
- Preserve `generated_at` from the existing file. Refresh `prd_path` to `{PRD_PATH}`.
- Apply the merge contract per feature ID (IDs are immutable):
  - **ID exists in old AND new PRD, old `status` is NOT `removed`:** preserve `status`, `cycles`, `failure_reason`, `report_path`, `started_at`, `completed_at` from existing entry. Refresh `name`, `priority`, `wave`, `dependencies` from the PRD. Bump `updated_at` to `now` ONLY if any refreshed field actually changed; otherwise keep the prior `updated_at`.
  - **ID exists in old AND new PRD, old `status` IS `removed` (feature resurrected):** the lifecycle restarts. Reset `status: "pending"`, `cycles: 0`, and clear all optional fields (`failure_reason`, `report_path`, `started_at`, `completed_at`) to `null`. Refresh `name`, `priority`, `wave`, `dependencies` from the PRD. Set `updated_at: now`.
  - **ID in new PRD only (never seen before):** add new entry exactly as in Case A (`pending`, `cycles: 0`, all optional fields `null`, `updated_at: now`).
  - **ID in old file only (feature removed from PRD):** keep the entry; if its `status` is not already `removed`, set `status: "removed"` and bump `updated_at: now`. Preserve `failure_reason`, `report_path`, `started_at`, `completed_at`, `cycles` as forensic record of the prior state. Never delete entries.

**Step 3 — Atomic write**

Write the JSON to `{PROGRESS_PATH}.tmp`, then rename to `{PROGRESS_PATH}`. This guarantees other skills never observe a partially written file.

**Step 4 — Verify and report**

- Read `{PROGRESS_PATH}` and confirm it parses as valid JSON and contains every feature ID from Section 6 of the PRD as a key in `features`.
- Inform the user of both paths: PRD and progress file.
...
- Save the progress file alongside the PRD in PHASE 5
- Use atomic write (`.tmp` + rename) for the progress file
- Apply the PHASE 5 merge contract when regenerating: preserve runtime state (`status`, `cycles`, `failure_reason`, `report_path`, `started_at`, `completed_at`) for features still present in the new PRD; reset to `pending` when a `removed` feature is resurrected; mark vanished features as `removed`
...
- Overwrite the progress file without applying the merge contract
- Delete entries from the progress file (set `status: "removed"` instead)
...
**Existing progress file is corrupt:**
- If `{PROGRESS_PATH}` exists but does not parse as valid JSON: do NOT silently overwrite it. Rename it to `{PROGRESS_PATH}.broken-<unix-timestamp>` and create a fresh file from the new PRD as in Case A. Inform the user that the prior file was preserved under the renamed path.

**Progress file schema version is newer than this skill knows:**
- If the existing file has `schema_version` greater than `1`: stop and report to the user. Newer state from a future skill version must not be downgraded. Do not write anything.

---

## PROGRESS FILE SCHEMA

The progress file (`prd_progress.json`) is a deterministic, machine-readable record of feature implementation status. This skill creates and merges it; downstream skills (`implement-feature`, `evaluator`, `fix-runner`, `implement-and-evaluate`) read and write to it during their own runs.
...
```json
{
  ...
  "generated_at": "<RFC 3339 UTC timestamp, set on first creation only>",
  "features": {
    "F01": { ... },
    "F02": { ... }
  }
...
- `name` / `priority` / `wave` / `dependencies` — copied from the PRD's Section 8 dependency table. Refreshed on every PRD regeneration.
- `status` — one of:
  - `pending` (terminal) — not started
  - `implementing` (transient) — `implement-feature` is actively running. Set on Step 5 start; replaced by `implemented` when implement-feature finishes successfully (or by `fail` on abort). You will rarely see this in a paused JSON — it persists only for the duration of the implement-feature run.
  - `implemented` (terminal-checkpoint) — implement-feature finished its work; the feature is in the implement → evaluate → fix loop awaiting a terminal verdict from the evaluator. Persists across skill invocations.
  - `done` (terminal) — evaluator passed; contract honored
  - `fail` (terminal) — evaluator failed terminally OR implement-feature aborted irrecoverably OR orchestrator exhausted retry budget / tripped circuit breaker
  - `pr-blocked` (terminal) — feature was successfully implemented and validated (eval clean), but the orchestrator could not open the pull request because merging the project's default branch into the feature branch produced conflicts that `fix-runner` could not resolve automatically. Distinct from `fail` because the implementation work itself is sound — only integration with main is blocked. Resolution is human: rebase/merge manually, push, then re-invoke or open the PR by hand.
  - `removed` (terminal) — feature existed in a prior PRD revision but is no longer in the current PRD
- `cycles` — count of `fix-runner` cycles consumed for this feature. Used by `implement-and-evaluate` to enforce the retry budget.
- `failure_reason` — required when `status: "fail"` or `status: "pr-blocked"`. Short string (under 200 chars) naming the cause of the failure (e.g., `"3 contract item(s) failed; first: API-UPLOAD-03 — endpoint returned 500"`, `"cycle budget exhausted (5 cycles); last eval: ..."`, `"unresolvable merge conflict in apps/web/lib/session.ts, apps/backend/src/main.ts"`). May also persist on `status: "removed"` as a forensic record of the prior failure. `null` for all other statuses (`pending`, `implementing`, `implemented`, `done`).
- `report_path` — optional pointer to the latest `eval-report-<ts>.md` for this feature. May be set on `done` (last successful eval) or `fail` (last failing eval), and persists on `removed` as forensic record. `null` when no eval has run.
- `started_at` — RFC 3339 UTC timestamp when status first left `pending`. `null` until then. Reset to `null` only when a `removed` feature is resurrected into a new PRD revision (a fresh lifecycle begins).
- `updated_at` — RFC 3339 UTC timestamp of the last write to this entry. Always set.
- `completed_at` — RFC 3339 UTC timestamp set when status becomes `done`. `null` otherwise.

**Invariants:**

- Every feature ID present in Section 6 of the PRD MUST exist as a key in `features`. The reverse is not required (`removed` entries persist beyond the current PRD).
- `cycles >= 0`.
- Every ID in `dependencies` must exist as a key in `features` (current or `removed`).
- All timestamps are UTC, RFC 3339 (`2026-05-02T14:30:00Z`).
